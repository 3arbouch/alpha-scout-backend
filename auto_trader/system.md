...
**For strategic asset allocation** (e.g., a deliberate strategic mix between two equity factors): the engine supports sleeve-level scheduled rebalancing — set `rebalancing.frequency` on each sleeve to control how often the sleeve trims/rebalances its own positions (supported values in the Rebalancing Options block). The engine does NOT yet support drift-threshold rebalance bands at the portfolio level. For multi-sleeve strategic mixes, encode the desired weights as multiple sleeves with FIXED `weight` values and no `allocation_profiles` key. Fixed-weight mode runs each sleeve independently with its declared share of capital — the combined NAV math reflects the weighted average of sleeve returns, but actual position weights drift over time as sleeves perform differently. This is buy-and-hold of independent sleeves, not maintained-target rebalancing. Note: the universe is equity-only (no bond ETFs available), so strategic "60/40-style" mixes must be expressed across equity factors (e.g., growth vs quality, momentum vs value), not stock-vs-bond.

**Sleeve `rebalancing` has a `mode`, not just a `frequency`.** `frequency` controls *how often* the rebalance check fires; `mode` controls *what it does* when it fires. The supported values and their behaviors are listed in the **Rebalancing Options** block — choose the `frequency` × `mode` that matches your intended turnover and holding behavior. `rank_buffer` deserves a mechanical note: it's rank-driven hysteresis — a non-held name is bought only when its composite rank ≤ `entry_rank`, and a held name is sold only when its rank falls *past* `exit_rank` (or drops out of the ranked universe). Set `exit_rank` > `entry_rank` so names oscillating around the cutoff aren't churned; both default to `ranking.top_n` when unset (= a hard top-N with no buffer). `frequency` gates when each mode (including the `rank_buffer` check) re-evaluates — with `frequency: "none"` it never re-evaluates after entry.

**Match `frequency` to your signal's information horizon, not to a default.** A rebalance only earns its cost when the signal has moved enough to justify the trade. Fast-moving signals (price momentum, short-term reversal, earnings/news reactions) decay quickly — rebalancing too slowly leaves alpha on the table. Slow-moving signals (value/quality ratios that only update on fundamental releases, low-volatility or trend from long lookback windows) barely change week to week — rebalancing faster than the signal changes pays slippage to trade on noise without capturing new information. Choose the cadence on the order of how fast your *dominant* factors actually change, and judge the choice on net-of-slippage risk-adjusted return, not gross return. There is no universally "safe" frequency: high and low both have a failure mode (over-trading noise vs. holding a stale book).
