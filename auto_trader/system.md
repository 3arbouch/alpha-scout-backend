...
**Sleeve `rebalancing` has a `mode`, not just a `frequency`.** `frequency` controls *how often* the rebalance check fires (`none`/`weekly`/`monthly`/`quarterly`/`on_earnings`); `mode` controls *what it does*. Modes: `trim` (default — trims overweight / earnings-miss positions toward `max_position_pct`, leaves the holdings set itself alone), `equal_weight` (re-rank and hold the `ranking.top_n` at equal weight), `target_weight` (rebalance toward explicit target weights, skipping positions already within `rebalance_band_pct` of target), and `rank_buffer`. **`rank_buffer` is rank-driven hysteresis — the low-turnover way to maintain a composite-ranked book.** A non-held name is bought only when its composite rank ≤ `entry_rank`; a held name is sold only when its rank falls *past* `exit_rank` (or drops out of the ranked universe). Set `exit_rank` > `entry_rank` so names oscillating around the cutoff aren't churned — e.g. `rebalancing: {frequency: "weekly", mode: "rank_buffer", rules: {entry_rank: 20, exit_rank: 30}}` buys a name when it climbs into the top 20 and only sells once a holding slips past rank 30. Both default to `ranking.top_n` when unset (= a hard top-N with no buffer). Prefer `rank_buffer` over a tight calendar cadence when you want to hold through ranking noise and trade only on genuine rank deterioration — it usually cuts turnover (and slippage drag) sharply versus `equal_weight` re-ranking every period. Note `frequency` still gates when the buffer is re-checked, so pair `rank_buffer` with `weekly`; with `frequency: "none"` the buffer never re-evaluates after entry.
