    ...
    try:
        ...
        conn.execute(
            """UPDATE deployments SET
               ...
               last_sharpe_ratio = ?, last_sharpe_basis = ?,
               last_sharpe_ratio_annualized = ?,
...
def daily_pnl_series(deploy_id: str) -> dict | None:
    """Slim daily P&L series for a deployment, with the market benchmark.

    One point per trading day: portfolio NAV + daily/cumulative return, and the
    market benchmark (e.g. SPY) rebased to the same initial capital and aligned
    to the same date grid as the portfolio. Benchmark fields are null on days
    with no benchmark point (e.g. multi-sector portfolios that carry no single
    market benchmark).

    Derived from the deployment's latest results.json (regenerated by each
    nightly evaluate). The response shape is intentionally stable so it can
    later be backed by a persisted daily table without a frontend change.

    `daily_pnl` / `daily_pnl_pct` are the engine's own day-over-day values
    (position_book): dollar change and % return on the prior day's NAV.
    """
    ...
    initial_capital = d.get("initial_capital") or (
        nav_history[0]["nav"] if nav_history else 0)

    bench_by_date = {p["date"]: p["nav"]
                     for p in (d.get("benchmark_nav") or [])}
    bench_market = d.get("benchmark_market")
    bench_symbol = (bench_market.get("symbol")
                    if isinstance(bench_market, dict) else None)

    series = []
    prev_bench = None
    for p in nav_history:
        nav = p.get("nav")
        b = bench_by_date.get(p["date"])
        if b is not None:
            bench_daily_pct = ((b / prev_bench - 1.0) * 100.0) if prev_bench else 0.0
            bench_cum_pct = ((b / initial_capital - 1.0) * 100.0) if initial_capital else None
            prev_bench = b
        else:
            bench_daily_pct = bench_cum_pct = None
        series.append({
            "date": p["date"],
            "nav": nav,
            "daily_pnl": p.get("daily_pnl"),
            "daily_pnl_pct": p.get("daily_pnl_pct"),
            "cum_return_pct": (round((nav / initial_capital - 1.0) * 100.0, 4)
                               if initial_capital and nav is not None else None),
            "benchmark_nav": b,
            "benchmark_daily_pct": (round(bench_daily_pct, 4)
                                    if bench_daily_pct is not None else None),
            "benchmark_cum_pct": (round(bench_cum_pct, 4)
                                  if bench_cum_pct is not None else None),
        })
...
def build_position_book(sleeves: list[dict], initial_capital: float) -> dict:
